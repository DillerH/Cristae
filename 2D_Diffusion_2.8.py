...
class ParticleGroup:
    ...
    def spawnRandParticle(self):
        ...
        if np.random.choice([True,False],p=[chanceAdd, chancePass]) == True:
            ...
            if self.label == 'Q10':
                Qrat = Qratio(self)
                if np.random.choice([True,False],p=[Qrat,1-Qrat])and np.random.rand()<=0.95:
                    redoxnum = self.popped.pop(0)
                    ...
                    self.particles = np.append(self.particles,Particle(0, 0))
                    self.redox = np.append(self.redox, 2)
                    self.oob = np.append(self.oob, 0)
                    self.numParticles += 1
    ...
    def removeOOB(self, t): #fix gauss incorrect if statement
        ...
        for k, particle in enumerate(oobParticles):
            ...
            #print(particle.position[1])
                ...
                if keepBool == False:
                    self.popped.append(particle.redox)
...
def plotFrame(substrateGroup, proteinGroupI, proteinGroupIII, time):
    ...
    ax.legend()
    plt.savefig('100Sq_I1III4Q100_Scatter.png',dpi=600)
    plt.show()
...
def plotQPool(Ratio):
    ...
    plt.savefig('100Sq_I1III4Q100_QRatio.png',dpi=600)
    plt.legend()
    plt.tight_layout()
    plt.show()
...
def plotQNums(t, y):
    ...
    plt.savefig('100Sq_I1III4Q100_QFlux.png',dpi=600)
    plt.show()
...
def makeMovie():
    ...
    ComplexIlist = list(Particle(1, 1) for i in range(0,1)) 
    ComplexIIIlist = list(Particle(1, 1) for i in range(0,10)) 
    
    Q =ParticleGroup(Qlist, 0.5, 8.2*10**6, 'Q10')
    ComplexI = ParticleGroup(ComplexIlist, 0.5, 7.8*10**5, 'CI')
    ComplexIII = ParticleGroup(ComplexIIIlist, 0.5, 1.0*10**6, 'CIII')
    
    Q.initRandFrame(100)
    ComplexI.initRandFrame(100)
    ComplexIII.initRandFrame(100)
    ...
    with writer.saving(fig, 'Diffusion5.mp4',100):
...
def main():
    ...
    Q10list = list(Particle(0, 1) for i in range(0,100))
    CIlist = list(Particle(1, 1) for i in range(0,1)) 
    CIIIlist = list(Particle(1, 1) for i in range(0,30)) 
    ...
    for i in range(stepCount):
        ...
        if printData == True and i % 100000 == 0 :
