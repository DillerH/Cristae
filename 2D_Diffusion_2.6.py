...
def makeMovie():
    ...
    for i in range(10):
        #Q10.spawnRandParticle(Particle(0,1), 200)
        Q10.randStep()
        CI.randStep()
        CIII.randStep()
        Q10.checkBox(frameSizeMax)
        CI.checkBox(frameSizeMax)
        CIII.checkBox(frameSizeMax)
        CI.reductionActivity(Q10, 25)
        CIII.oxidationActivity(Q10, 15)
        
        Q0eList, Q1eList, Q2eList = [], [], []
        for Q in Q10.particles:
            #print(Q.redox)
            if Q.redox == 0:
                Q0eList.append(Q)
            if Q.redox == 1:
        ...
        #print(Q0e.positions)
        plt.scatter(CI.positions[:,0], CI.positions[:,1],s=200, label = 'CI', color = 'crimson')
        plt.scatter(CIII.positions[:,0], CIII.positions[:,1],s=80, label = 'CIII', color = 'slateblue')
        if Q0ePresent:
            plt.scatter(Q0e.positions[:,0], Q0e.positions[:,1], label = 'CoQ',s=20, color = 'lightgray')
        if Q1ePresent:
            plt.scatter(Q1e.positions[:,0], Q1e.positions[:,1], label = 'SQ',s=20, color = 'dimgray')
        if Q2ePresent:
            plt.scatter(Q2e.positions[:,0], Q2e.positions[:,1], label = r'QH$_2$',s=20, color = 'black') 
        plt.xlabel('x (nm)')
        plt.ylabel('y (nm)')
        plt.title('Reduction Activity of Simulated Particles after '+str(time/1000)+' ms')
